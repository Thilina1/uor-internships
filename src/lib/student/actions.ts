...
export async function updateStudentProfile(formData: { name: string; mobile_number: string }) {
...
export async function submitInquiry(formData: { subject: string; message: string }) {
    const session = await getSession();
    if (!session?.user) {
        return { error: "Not authenticated" };
    }
    ...
    if (session.user.role !== "student") {
        return { error: "Only students can submit inquiries" };
    }
    ...
    revalidatePath("/dashboard/student");
    revalidatePath("/dashboard/student/inquiries");
    return { success: true };
}
